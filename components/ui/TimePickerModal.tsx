...
export function TimePickerModal({ visible, onClose, selectedTime24, onSelectTime }: TimePickerModalProps) {
  ...
  return (
    <AppModal
      visible={visible}
      onClose={() => { setHourDropdownOpen(false); setMinuteDropdownOpen(false); onClose(); }}
      animationType="fade"
    >
      <Pressable
        className="bg-white rounded-3xl p-6 border border-zinc-200 w-full max-w-sm shadow-xl"
